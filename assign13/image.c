...
//==================================================================
struct pixelType cvt_pixel_d(struct pixelType orig_p, struct pixelType sec_p, double blend)
{
   ...
   new_p.red = r*blend+r2*(1-blend);
   new_p.green = g*blend+g2*(1-blend);
   new_p.blue = b*blend+b2*(1-blend);

   return new_p;
}
//==================================================================
struct pixelType cvt_pixel_b(struct pixelType orig_p, struct pixelType sec_p)
{
   unsigned char r,g,b,r2,g2,b2;
   struct pixelType new_p;
   r = orig_p.red;
...
void cvt_d(struct imageType *orig_img, struct imageType *second_img, struct imageType *new_img)
{
   ...
   new_img->height = orig_img->height;
   new_img->maxColor = orig_img->maxColor;
   printf("Please enter blending factor in decimal form: ");
   scanf("%lf", &blendfactor);
   while(blendfactor < 0&&blendfactor>1){
      printf("Please enter blending factor in decimal form: ");
      scanf("%lf", &blendfactor);
      printf("\n");
   }
   for(y=new_img->height-1;y>=0; y--) 
      for(x=0; x<new_img->width; x++) {
         new_img->image[y][x] = cvt_pixel_d(orig_img->image[y][x], second_img->image[y][x], blendfactor);
    }
}
//==================================================================
void cvt_b(struct imageType *orig_img, struct imageType *second_img, struct imageType *new_img)
{
   ...
   strcpy(new_img->magicNumber, orig_img->magicNumber);
   strcpy(new_img->comment,orig_img->comment);
   new_img->width = second_img->width;
   new_img->height = second_img->height;
   new_img->maxColor = second_img->maxColor;
   for(y=new_img->height-1;y>=0; y--) 
      for(x=0; x<new_img->width; x++) {
         new_img->image[y][x] = cvt_pixel_b(orig_img->image[y][x], second_img->image[y][x]);
    }
}
